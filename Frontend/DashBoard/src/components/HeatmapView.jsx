...
const OWM_KEY = "9c9815bb312b7d2d7b1dda93051932a5";
const OWM_URL = (lat, lon) =>
  `https://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${OWM_KEY}`;

// ─── AQI Color Scale (CPCB Indian Standard) ───────────────────────────────────
const AQI_SCALE = [
...
const subIndex = (val, poll) => {
  const bps = BP[poll]; if (!bps) return 0;
  for (const [cl, ch, il, ih] of bps) {
    if (val >= cl && val <= ch) return il + ((ih - il) / (ch - cl)) * (val - cl);
  }
  return 500;
};

const calcAQI = (c) => {
  const coMg = (c.co || 0) / 1000;
  return Math.round(Math.max(
    subIndex(c.pm2_5 || 0, "pm2_5"),
    subIndex(c.pm10 || 0, "pm10"),
    subIndex(c.no2 || 0, "no2"),
    subIndex(c.so2 || 0, "so2"),
    subIndex(coMg, "co"),
  ));
};
...
const fetchAQI = async (lat, lon) => {
  const res = await fetch(OWM_URL(lat, lon));
  ...
  return {
    ...
    bg: lvl.bg,
    text: lvl.text,
    owm_aqi: json.list[0].main.aqi,   // OWM 1-5 scale
    pm2_5: c.pm2_5,
    pm10: c.pm10,
    no2: c.no2,
    so2: c.so2,
...
const HeatmapView = ({ lat, lon }) => {
  ...
  const [selected, setSelected] = useState(null); // { data, name }
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (results.length === 0) setError("Could not reach OpenWeatherMap. Check API key or network.");
      setLoading(false);
    };
    load();
  }, []);
  ...
  if (loading && cities.length === 0) {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 500, gap: 16 }}>
        ...
        <p style={{ fontWeight: 700, color: "#475569" }}>Fetching AQI for {CITIES.length} cities…</p>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        {[
          ...
          { label: "Cities", value: cities.length, sub: loading ? `${loaded}/${CITIES.length} loading…` : "All states" },
        ...
        >
          ...
          {cities.map((c) => (
            <CircleMarker
              ...
              eventHandlers={{
                click: () => setSelected({ data: c, name: c.name }),
              }}
            >
              <Popup>
 ...
 AQI: {clickResult.aqi} — {clickResult.label}
        ...
        <div style={{
          ...
          <p style={{ fontSize: 9, fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: 1, margin: "0 0 6px" }}>CPCB AQI</p>
          {AQI_SCALE.map((l) => (
        ...
        {clickLoading && (
          <div style={{
            ...
            <span style={{ fontSize: 12, fontWeight: 600, color: "white" }}>Fetching AQI…</span>
          </div>
        )}
